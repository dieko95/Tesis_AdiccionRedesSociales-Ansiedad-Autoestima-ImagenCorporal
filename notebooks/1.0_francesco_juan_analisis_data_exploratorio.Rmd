...
run_quantitative_bivariate_test <-
  function(xColName,
           ...
           y_label = "",
           title = "") {
    x_vector_pvalue <- shapiro.test(original_df[[xColName]])
    y_vector_pvalue <- shapiro.test(original_df[[yColName]])
    
    if ((y_vector_pvalue$p.value > 0.05) & (x_vector_pvalue$p.value > 0.05)) {
      method <- "pearson"
    } else {
      method <- "spearman"
    }
    ...
    cor_test_result <-
      cor.test(original_df[[xColName]],
               original_df[[yColName]],
               method = method)
    ...
    df %>%
      ggplot(aes(x = !!rlang::sym(xColName), y = !!rlang::sym(yColName))) +
      geom_point() +
...
plot_grid(ars_out, ars_out1,ars_out2, ars_out3)
```
...
```{r}
      ...
      plot_grid(bsq_out1, bsq_out2, bsq_out3, ncol = 3)
```
...
#ANALISIS POR FACTORES DE ARS

# Igualdad de varianzas: si p > 0.05 supuesto se cumple
LeveneTest(original_df$obsesion_rrss, original_df$municipio)

# Normalidad de VD: si p > 0.05 supuesto se cumple
shapiro.test(original_df$obsesion_rrss)

bi_mun2 <- ggbetweenstats(
  data  = original_df,
  x     = municipio,
  y     = obsesion_rrss,
  outlier.tagging  = TRUE,
  type = "nonparametric", # cambiar a "parametric" si supuestos se cumplen.
  title = "Obesión de las Redes Sociales Según municipio" # Titulo con variables.
)+ coord_flip()

# Igualdad de varianzas: si p > 0.05 supuesto se cumple
LeveneTest(original_df$falta_de_control_rrss, original_df$municipio)

# Normalidad de VD: si p > 0.05 supuesto se cumple
shapiro.test(original_df$falta_de_control_rrss)

bi_mun3 <- ggbetweenstats(
  data  = original_df,
  x     = municipio,
  y     = falta_de_control_rrss,
  outlier.tagging  = TRUE,
  type = "nonparametric", # cambiar a "parametric" si supuestos se cumplen.
  title = "Falta de control de las Redes Sociales Según municipio" # Titulo con variables.
)+ coord_flip()

# Igualdad de varianzas: si p > 0.05 supuesto se cumple
LeveneTest(original_df$uso_excesivo_rrss, original_df$municipio)

# Normalidad de VD: si p > 0.05 supuesto se cumple
shapiro.test(original_df$uso_excesivo_rrss)

bi_mun4 <- ggbetweenstats(
  data  = original_df,
  x     = municipio,
  y     = uso_excesivo_rrss,
  outlier.tagging  = TRUE,
  type = "parametric", # cambiar a "parametric" si supuestos se cumplen.
  title = "Uso excesivo de las redes sociales Según municipio" # Titulo con variables.
)+ coord_flip()

 plot_grid(bi_mun1, bi_mun2, bi_mun3, bi_mun4)
```
...
# Igualdad de varianzas: si p > 0.05 supuesto se cumple
LeveneTest(original_df$obsesion_rrss, original_df$sexo)

# Normalidad de VD: si p > 0.05 supuesto se cumple
shapiro.test(original_df$obsesion_rrss)

bi_sexo2 <- ggbetweenstats(
  data = original_df %>%
  filter(sexo != "No binario"),
  x     = sexo,
  y     = obsesion_rrss,
  outlier.tagging  = TRUE,
  type = "nonparametric", # cambiar a "parametric" si supuestos se cumplen.
  title = "Obesión de las Redes Sociales Según Sexo" # Titulo con variables.
) 

# Igualdad de varianzas: si p > 0.05 supuesto se cumple
LeveneTest(original_df$falta_de_control_rrss, original_df$sexo)

# Normalidad de VD: si p > 0.05 supuesto se cumple
shapiro.test(original_df$falta_de_control_rrss)

bi_sexo3 <- ggbetweenstats(
  data = original_df %>%
  filter(sexo != "No binario"),
  x     = sexo,
  y     = falta_de_control_rrss,
  outlier.tagging  = TRUE,
  type = "nonparametric", # cambiar a "parametric" si supuestos se cumplen.
  title = "Falta de control de las Redes Sociales Según Sexo" # Titulo con variables.
) 

# Igualdad de varianzas: si p > 0.05 supuesto se cumple
LeveneTest(original_df$uso_excesivo_rrss, original_df$sexo)

# Normalidad de VD: si p > 0.05 supuesto se cumple
shapiro.test(original_df$uso_excesivo_rrss)

bi_sexo4 <- ggbetweenstats(
  data = original_df %>%
  filter(sexo != "No binario"),
  x     = sexo,
  y     = uso_excesivo_rrss,
  outlier.tagging  = TRUE,
  type = "parametric", # cambiar a "parametric" si supuestos se cumplen.
  title = "Uso excesivo de las redes sociales Según Sexo" # Titulo con variables.
) 

plot_grid(bi_sexo1,bi_sexo2,bi_sexo3, bi_sexo4)
```
...
run_quantitative_bivariate_test(
  xColName = "edad",
  yColName = "adiccion_rrss",
  df = original_df,
  x_label = "Edad",
  y_label = "Factor Obsesión",
  title = "Relación entre la edad y factor Obsesión"
)

run_quantitative_bivariate_test(
  xColName = "edad",
  yColName = "obsesion_rrss",
  df = original_df,
  x_label = "Edad",
  y_label = "Factor Obsesión",
  title = "Relación entre la edad y factor Obsesión"
...
run_quantitative_bivariate_test(
  xColName = "edad",
  yColName = "falta_de_control_rrss",
  df = original_df,
  x_label = "Edad",
  y_label = "Factor Obsesión",
  title = "Relación entre la edad y factor Obsesión"
...
# Igualdad de varianzas: si p > 0.05 supuesto se cumple
LeveneTest(original_df$obsesion_rrss, original_df$educación)

# Normalidad de VD: si p > 0.05 supuesto se cumple
shapiro.test(original_df$obsesion_rrss)

bi_edu2 <- ggbetweenstats(
  data  = original_df,
  x     = educación,
  y     = obsesion_rrss,
  outlier.tagging  = TRUE,
  type = "nonparametric", # cambiar a "parametric" si supuestos se cumplen.
  title = "Obesión de las Redes Sociales Según Nivel Educativo" # Titulo con variables.
) 

# Igualdad de varianzas: si p > 0.05 supuesto se cumple
LeveneTest(original_df$falta_de_control_rrss, original_df$educación)

# Normalidad de VD: si p > 0.05 supuesto se cumple
shapiro.test(original_df$falta_de_control_rrss)

bi_edu3 <- ggbetweenstats(
  data  = original_df,
  x     = educación,
  y     = falta_de_control_rrss,
  outlier.tagging  = TRUE,
  type = "nonparametric", # cambiar a "parametric" si supuestos se cumplen.
  title = "Falta de control de las Redes Sociales Según Nivel Educativo" # Titulo con variables.
) 

# Igualdad de varianzas: si p > 0.05 supuesto se cumple
LeveneTest(original_df$uso_excesivo_rrss, original_df$educación)

# Normalidad de VD: si p > 0.05 supuesto se cumple
shapiro.test(original_df$uso_excesivo_rrss)

bi_edu4 <- ggbetweenstats(
  data  = original_df,
  x     = educación,
  y     = uso_excesivo_rrss,
  outlier.tagging  = TRUE,
  type = "parametric", # cambiar a "parametric" si supuestos se cumplen.
  title = "Uso excesivo de las redes sociales Según Nivel Educativo" # Titulo con variables.
)

plot_grid(bi_edu1, bi_edu2, bi_edu3, bi_edu4)
```
...
# Igualdad de varianzas: si p > 0.05 supuesto se cumple
LeveneTest(original_df$obsesion_rrss, original_df$tipo_universidad)
...
ggbetweenstats(
  ...
  outlier.tagging  = TRUE,
  type = "nonparametric", # cambiar a "parametric" si supuestos se cumplen.
  title = "Obesión de las Redes Sociales Según Tipo de Universidad" # Titulo con variables.
)

# Igualdad de varianzas: si p > 0.05 supuesto se cumple
LeveneTest(original_df$falta_de_control_rrss, original_df$tipo_universidad)
...
ggbetweenstats(
  ...
  outlier.tagging  = TRUE,
  type = "nonparametric", # cambiar a "parametric" si supuestos se cumplen.
  title = "Falta de control de las Redes Sociales Según Tipo de Universidad" # Titulo con variables.
)

# Igualdad de varianzas: si p > 0.05 supuesto se cumple
LeveneTest(original_df$uso_excesivo_rrss, original_df$tipo_universidad)
...
ggbetweenstats(
  ...
  outlier.tagging  = TRUE,
  type = "parametric", # cambiar a "parametric" si supuestos se cumplen.
  title = "Uso excesivo de las redes sociales Según Tipo de Universidad" # Titulo con variables.
)
```
...
# Igualdad de varianzas: si p > 0.05 supuesto se cumple
LeveneTest(original_df$obsesion_rrss, original_df$trabajo_actual)

# Normalidad de VD: si p > 0.05 supuesto se cumple
shapiro.test(original_df$obsesion_rrss)

bi_ocupa2 <- ggbetweenstats(
  data  = original_df,
  x     = trabajo_actual,
  y     = obsesion_rrss,
  outlier.tagging  = TRUE,
  type = "nonparametric", # cambiar a "parametric" si supuestos se cumplen.
  title = "Obesión de las Redes Sociales Según la Ocupación" # Titulo con variables.
)

# Igualdad de varianzas: si p > 0.05 supuesto se cumple
LeveneTest(original_df$falta_de_control_rrss, original_df$trabajo_actual)

# Normalidad de VD: si p > 0.05 supuesto se cumple
shapiro.test(original_df$falta_de_control_rrss)

bi_ocupa3 <- ggbetweenstats(
  data  = original_df,
  x     = trabajo_actual,
  y     = falta_de_control_rrss,
  outlier.tagging  = TRUE,
  type = "nonparametric", # cambiar a "parametric" si supuestos se cumplen.
  title = "Falta de control de las Redes Sociales Según la Ocupación" # Titulo con variables.
)

# Igualdad de varianzas: si p > 0.05 supuesto se cumple
LeveneTest(original_df$uso_excesivo_rrss, original_df$trabajo_actual)

# Normalidad de VD: si p > 0.05 supuesto se cumple
shapiro.test(original_df$uso_excesivo_rrss)

bi_ocupa4 <- ggbetweenstats(
  data  = original_df,
  x     = trabajo_actual,
  y     = uso_excesivo_rrss,
  outlier.tagging  = TRUE,
  type = "nonparametric", # cambiar a "parametric" si supuestos se cumplen.
  title = "Uso excesivo de las redes sociales Según la Ocupación" # Titulo con variables.
)

plot_grid(bi_ocupa1, bi_ocupa2, bi_ocupa3, bi_ocupa4)
```
...
# Igualdad de varianzas: si p > 0.05 supuesto se cumple
LeveneTest(original_df$obsesion_rrss, original_df$top1_rrss)

# Normalidad de VD: si p > 0.05 supuesto se cumple
shapiro.test(original_df$obsesion_rrss)

bi_top2 <- ggbetweenstats(
  data  = original_df,
  x     = top1_rrss,
  y     = obsesion_rrss,
  outlier.tagging  = TRUE,
  type = "nonparametric", # cambiar a "parametric" si supuestos se cumplen.
  title = "Obesión Según top 1 de Redes Sociales" # Titulo con variables.
)

# Igualdad de varianzas: si p > 0.05 supuesto se cumple
LeveneTest(original_df$falta_de_control_rrss, original_df$top1_rrss)

# Normalidad de VD: si p > 0.05 supuesto se cumple
shapiro.test(original_df$falta_de_control_rrss)

bi_top3 <- ggbetweenstats(
  data  = original_df,
  x     = top1_rrss,
  y     = falta_de_control_rrss,
  outlier.tagging  = TRUE,
  type = "nonparametric", # cambiar a "parametric" si supuestos se cumplen.
  title = "Falta de control Según top 1 de Redes Sociales" # Titulo con variables.
)

# Igualdad de varianzas: si p > 0.05 supuesto se cumple
LeveneTest(original_df$uso_excesivo_rrss, original_df$top1_rrss)

# Normalidad de VD: si p > 0.05 supuesto se cumple
shapiro.test(original_df$uso_excesivo_rrss)

bi_top4 <- ggbetweenstats(
  data  = original_df,
  x     = top1_rrss,
  y     = uso_excesivo_rrss,
  outlier.tagging  = TRUE,
  type = "parametric", # cambiar a "parametric" si supuestos se cumplen.
  title = "Uso excesivo Según top 1 de Redes Sociales" # Titulo con variables.
)

plot_grid(bi_top1,bi_top2, bi_top3, bi_top4)
```
...
#Constructo
p1 <- original_df %>% 
  ggplot(aes(x=adiccion_rrss, fill = adiccion_rrss)) +
    geom_histogram() + 
    labs(x= "Puntaje normalizado", y= "Frecuencia", title = "Constructo Adicción a las Redes Sociales")
original_df %>%
  select(adiccion_rrss, obsesion_rrss, falta_de_control_rrss, uso_excesivo_rrss)%>%
  psych::describe()

#Obsesión
p2 <- original_df %>% 
  ggplot(aes(x=obsesion_rrss, fill = obsesion_rrss)) +
    geom_histogram() + 
    labs(x= "Puntaje normalizado", y= "Frecuencia", title = "Factor Obsesión")

#Falta de control
p3 <- original_df %>% 
  ggplot(aes(x=falta_de_control_rrss, fill = falta_de_control_rrss)) +
    geom_histogram() + 
    labs(x= "Puntaje normalizado", y= "Frecuencia", title = "Factor Falta de Control")

#Uso Excesivo
p4 <- original_df %>% 
  ggplot(aes(x=uso_excesivo_rrss, fill = uso_excesivo_rrss)) +
    geom_histogram() + 
    labs(x= "Puntaje normalizado", y= "Frecuencia", title = "Factor Uso Excesivo")
...
#área interpersonal
a1 <- original_df %>% 
  ggplot(aes(x= area_interperso, fill = area_interperso)) +
    geom_histogram() + 
    labs(x= "Puntaje normalizado", y= "Frecuencia", title = "Factor Área Interpersonal")

#área personal/corporal
a2 <- original_df %>% 
  ggplot(aes(x=area_personal_corporal, fill = area_personal_corporal)) +
    geom_histogram() + 
    labs(x= "Puntaje normalizado", y= "Frecuencia", title = "Factor Área Personal Corporal")

original_df %>%
  select(area_interperso, area_personal_corporal)%>%
  psych::describe()
...
b1 <- original_df %>% 
  ggplot(aes(x=insatisfaccion_imagen_corp, fill = insatisfaccion_imagen_corp)) +
    geom_histogram() + 
    labs(x= "Puntaje normalizado", y= "Frecuencia", title = "Factor Insatisfacción Corporal")

#Preocupación por el Peso Corporal y Conductas Asociadas
b2 <- original_df %>% 
  ggplot(aes(x=preocupacion_peso_conductas_asociadas, fill = preocupacion_peso_conductas_asociadas)) +
    geom_histogram() + 
    labs(x= "Puntaje normalizado", y= "Frecuencia", title = "Factor preocupación por el peso y conductas asociadas")

#Factores Antiestéticos
b3 <- original_df %>% 
  ggplot(aes(x=factores_antiesteticos_emociones, fill = factores_antiesteticos_emociones)) +
    geom_histogram() + 
    labs(x= "Puntaje normalizado", y= "Frecuencia", title = "Factor Antiestéticos y Emociones")

original_df %>%
  select(insatisfaccion_imagen_corp, preocupacion_peso_conductas_asociadas, factores_antiesteticos_emociones)%>%
  psych::describe()
...
# Igualdad de varianzas: si p > 0.05 supuesto se cumple
LeveneTest(original_df$obsesion_rrss, original_df$tipo_dispositivo_rrss)

# Normalidad de VD: si p > 0.05 supuesto se cumple
shapiro.test(original_df$obsesion_rrss)

bi_disp2 <- ggbetweenstats(
  data  = original_df,
  x     = tipo_dispositivo_rrss,
  y     = obsesion_rrss,
  outlier.tagging  = TRUE,
  type = "nonparametric", # cambiar a "parametric" si supuestos se cumplen.
  title = "Obesión de las Redes Sociales Según el Tipo de dispositivo de uso" # Titulo con variables.
) + (theme(axis.text.x = element_text(angle = 20, vjust=0.5, size = 5)))

# Igualdad de varianzas: si p > 0.05 supuesto se cumple
LeveneTest(original_df$falta_de_control_rrss, original_df$tipo_dispositivo_rrss)

# Normalidad de VD: si p > 0.05 supuesto se cumple
shapiro.test(original_df$falta_de_control_rrss)

bi_disp3 <- ggbetweenstats(
  data  = original_df,
  x     = tipo_dispositivo_rrss,
  y     = falta_de_control_rrss,
  outlier.tagging  = TRUE,
  type = "nonparametric", # cambiar a "parametric" si supuestos se cumplen.
  title = "Falta de control de las Redes Sociales Según el Tipo de dispositivo de uso" # Titulo con variables.
) + (theme(axis.text.x = element_text(angle = 20, vjust=0.5, size = 5)))

# Igualdad de varianzas: si p > 0.05 supuesto se cumple
LeveneTest(original_df$uso_excesivo_rrss, original_df$tipo_dispositivo_rrss)

# Normalidad de VD: si p > 0.05 supuesto se cumple
shapiro.test(original_df$uso_excesivo_rrss)

bi_disp4 <- ggbetweenstats(
  data = original_df,
  x     = tipo_dispositivo_rrss,
  y     = uso_excesivo_rrss,
  outlier.tagging  = TRUE,
  type = "parametric", # cambiar a "parametric" si supuestos se cumplen.
  title = "Uso excesivo de las redes sociales Según el Tipo de dispositivo de uso" # Titulo con variables.
) + (theme(axis.text.x = element_text(angle = 20, vjust=0.5, size = 5)))

plot_grid(bi_disp1,bi_disp2, bi_disp3, bi_disp4 )
```
...
bi_inter1 <- ggbetweenstats(
  data  = original_df,
  x     = internet_fijo,
  y     = adiccion_rrss,
  outlier.tagging  = TRUE,
  type = "parametric", # cambiar a "parametric" si supuestos se cumplen.
  title = "Adicción Redes Sociales Según el servicio fijo de internet" # Titulo con variables.
) 

#ANALISIS POR FACTORES DE ARS

# Igualdad de varianzas: si p > 0.05 supuesto se cumple
LeveneTest(original_df$obsesion_rrss, original_df$internet_fijo)

# Normalidad de VD: si p > 0.05 supuesto se cumple
shapiro.test(original_df$obsesion_rrss)

bi_inter2 <-ggbetweenstats(
  data  = original_df,
  x     = internet_fijo,
  y     = obsesion_rrss,
  outlier.tagging  = TRUE,
  type = "nonparametric", # cambiar a "parametric" si supuestos se cumplen.
  title = "Obesión de las Redes Sociales Según el servicio fijo de internet" # Titulo con variables.
)

# Igualdad de varianzas: si p > 0.05 supuesto se cumple
LeveneTest(original_df$falta_de_control_rrss, original_df$internet_fijo)

# Normalidad de VD: si p > 0.05 supuesto se cumple
shapiro.test(original_df$falta_de_control_rrss)

bi_inter3 <-ggbetweenstats(
  data  = original_df,
  x     = internet_fijo,
  y     = falta_de_control_rrss,
  outlier.tagging  = TRUE,
  type = "nonparametric", # cambiar a "parametric" si supuestos se cumplen.
  title = "Falta de control de las Redes Sociales Según el servicio fijo de internet" # Titulo con variables.
)

# Igualdad de varianzas: si p > 0.05 supuesto se cumple
LeveneTest(original_df$uso_excesivo_rrss, original_df$internet_fijo)

# Normalidad de VD: si p > 0.05 supuesto se cumple
shapiro.test(original_df$uso_excesivo_rrss)

bi_inter4 <-ggbetweenstats(
  data  = original_df,
  x     = internet_fijo,
  y     = uso_excesivo_rrss,
  outlier.tagging  = TRUE,
  type = "parametric", # cambiar a "parametric" si supuestos se cumplen.
  title = "Uso excesivo de las redes sociales Según el servicio fijo de internet" # Titulo con variables.
)

plot_grid(bi_inter1, bi_inter2, bi_inter3, bi_inter4)
```
...
# Igualdad de varianzas: si p > 0.05 supuesto se cumple
LeveneTest(original_df$obsesion_rrss, original_df$calidad_internet_fijo)

# Normalidad de VD: si p > 0.05 supuesto se cumple
shapiro.test(original_df$obsesion_rrss)

bi_cali2 <- ggbetweenstats(
  data  = original_df,
  x     = calidad_internet_fijo,
  y     = obsesion_rrss,
  outlier.tagging  = TRUE,
  type = "nonparametric", # cambiar a "parametric" si supuestos se cumplen.
  title = "Obesión de las Redes Sociales Según Calidad de Internet Fijo" # Titulo con variables.
)+coord_flip()

# Igualdad de varianzas: si p > 0.05 supuesto se cumple
LeveneTest(original_df$falta_de_control_rrss, original_df$calidad_internet_fijo)

# Normalidad de VD: si p > 0.05 supuesto se cumple
shapiro.test(original_df$falta_de_control_rrss)

bi_cali3 <- ggbetweenstats(
  data  = original_df,
  x     = calidad_internet_fijo,
  y     = falta_de_control_rrss,
  outlier.tagging  = TRUE,
  type = "nonparametric", # cambiar a "parametric" si supuestos se cumplen.
  title = "Falta de control de las Redes Sociales Según Calidad de Internet Fijo" # Titulo con variables.
)+coord_flip()

# Igualdad de varianzas: si p > 0.05 supuesto se cumple
LeveneTest(original_df$uso_excesivo_rrss, original_df$calidad_internet_fijo)

# Normalidad de VD: si p > 0.05 supuesto se cumple
shapiro.test(original_df$uso_excesivo_rrss)

bi_cali4 <- ggbetweenstats(
  data  = original_df,
  x     = calidad_internet_fijo,
  y     = uso_excesivo_rrss,
  outlier.tagging  = TRUE,
  type = "parametric", # cambiar a "parametric" si supuestos se cumplen.
  title = "Uso excesivo de las redes sociales Según Calidad de Internet Fijo" # Titulo con variables.
) +coord_flip()

plot_grid(bi_cali1, bi_cali2, bi_cali3, bi_cali4)
```
...
# Igualdad de varianzas: si p > 0.05 supuesto se cumple
LeveneTest(original_df$obsesion_rrss, original_df$fallas_servicio)

# Normalidad de VD: si p > 0.05 supuesto se cumple
shapiro.test(original_df$obsesion_rrss)

bi_falla2 <-ggbetweenstats(
  data  = original_df,
  x     = fallas_servicio,
  y     = obsesion_rrss,
  outlier.tagging  = TRUE,
  type = "nonparametric", # cambiar a "parametric" si supuestos se cumplen.
  title = "Obesión de las Redes Sociales Según falla del servicio fijo" # Titulo con variables.
)

# Igualdad de varianzas: si p > 0.05 supuesto se cumple
LeveneTest(original_df$falta_de_control_rrss, original_df$fallas_servicio)

# Normalidad de VD: si p > 0.05 supuesto se cumple
shapiro.test(original_df$falta_de_control_rrss)

bi_falla3 <-ggbetweenstats(
  data  = original_df,
  x     = fallas_servicio,
  y     = falta_de_control_rrss,
  outlier.tagging  = TRUE,
  type = "nonparametric", # cambiar a "parametric" si supuestos se cumplen.
  title = "Falta de control de las Redes Sociales Según falla del servicio fijo" # Titulo con variables.
)

# Igualdad de varianzas: si p > 0.05 supuesto se cumple
LeveneTest(original_df$uso_excesivo_rrss, original_df$fallas_servicio)

# Normalidad de VD: si p > 0.05 supuesto se cumple
shapiro.test(original_df$uso_excesivo_rrss)

bi_falla4 <-ggbetweenstats(
  data  = original_df,
  x     = fallas_servicio,
  y     = uso_excesivo_rrss,
  outlier.tagging  = TRUE,
  type = "parametric", # cambiar a "parametric" si supuestos se cumplen.
  title = "Uso excesivo de las redes sociales Según falla del servicio fijo" # Titulo con variables.
)

plot_grid(bi_falla1,bi_falla2, bi_falla3, bi_falla4)
```
...
# Igualdad de varianzas: si p > 0.05 supuesto se cumple
LeveneTest(original_df$obsesion_rrss, original_df$internet_fijo_positivo)
...
ggbetweenstats(
  ...
  outlier.tagging  = TRUE,
  type = "nonparametric", # cambiar a "parametric" si supuestos se cumplen.
  title = "Obesión de las Redes Sociales Según percepción positiva del servicio fijo" # Titulo con variables.
)

# Igualdad de varianzas: si p > 0.05 supuesto se cumple
LeveneTest(original_df$falta_de_control_rrss, original_df$internet_fijo_positivo)
...
ggbetweenstats(
  ...
  outlier.tagging  = TRUE,
  type = "nonparametric", # cambiar a "parametric" si supuestos se cumplen.
  title = "Falta de control de las Redes Sociales Según percepción positiva del servicio fijo" # Titulo con variables.
)

# Igualdad de varianzas: si p > 0.05 supuesto se cumple
LeveneTest(original_df$uso_excesivo_rrss, original_df$internet_fijo_positivo)
...
ggbetweenstats(
  ...
  outlier.tagging  = TRUE,
  type = "parametric", # cambiar a "parametric" si supuestos se cumplen.
  title = "Uso excesivo de las redes sociales Según percepción positiva del servicio fijo" # Titulo con variables.
)
```
...
**#Adicción a las Redes Sociales vs Percepción Negativa del Servicio**
...
# Igualdad de varianzas: si p > 0.05 supuesto se cumple
LeveneTest(original_df$obsesion_rrss, original_df$internet_fijo_negativo)
...
ggbetweenstats(
  ...
  outlier.tagging  = TRUE,
  type = "nonparametric", # cambiar a "parametric" si supuestos se cumplen.
  title = "Obesión de las Redes Sociales Según percepción percepción negativa del servicio fijo" # Titulo con variables.
) + coord_flip()

# Igualdad de varianzas: si p > 0.05 supuesto se cumple
LeveneTest(original_df$falta_de_control_rrss, original_df$internet_fijo_negativo)
...
ggbetweenstats(
  ...
  outlier.tagging  = TRUE,
  type = "nonparametric", # cambiar a "parametric" si supuestos se cumplen.
  title = "Falta de control de las Redes Sociales Según percepción percepción negativa del servicio fijo" # Titulo con variables.
) + coord_flip()

# Igualdad de varianzas: si p > 0.05 supuesto se cumple
LeveneTest(original_df$uso_excesivo_rrss, original_df$internet_fijo_negativo)
...
ggbetweenstats(
  ...
  outlier.tagging  = TRUE,
  type = "parametric", # cambiar a "parametric" si supuestos se cumplen.
  title = "Uso excesivo de las redes sociales Según percepción negativa del servicio fijo" # Titulo con variables.
) + coord_flip()
```
...
ggbetweenstats(
  data  = original_df %>% filter(
    fallo_internet_reestablecimiento %in% c(
      "Entre 1 y 2 días",
      "Entre 2 y 4 días",
      "El mismo día")),
...
# Igualdad de varianzas: si p > 0.05 supuesto se cumple
LeveneTest(original_df$obsesion_rrss, original_df$fallo_internet_reestablecimiento)
...
ggbetweenstats(
  ...
  outlier.tagging  = TRUE,
  type = "nonparametric", # cambiar a "parametric" si supuestos se cumplen.
  title = "Obesión de las Redes Sociales Según fallo del servicio fijo" # Titulo con variables.
) + (theme(axis.text.x = element_text(angle = 45, vjust=0.5, size = 8)))

# Igualdad de varianzas: si p > 0.05 supuesto se cumple
LeveneTest(original_df$falta_de_control_rrss, original_df$fallo_internet_reestablecimiento)
...
ggbetweenstats(
  ...
  outlier.tagging  = TRUE,
  type = "nonparametric", # cambiar a "parametric" si supuestos se cumplen.
  title = "Falta de control de las Redes Sociales Según fallo del servicio fijo" # Titulo con variables.
) + (theme(axis.text.x = element_text(angle = 45, vjust=0.5, size = 8)))

# Igualdad de varianzas: si p > 0.05 supuesto se cumple
LeveneTest(original_df$uso_excesivo_rrss, original_df$fallo_internet_reestablecimiento)
...
ggbetweenstats(
  ...
  outlier.tagging  = TRUE,
  type = "parametric", # cambiar a "parametric" si supuestos se cumplen.
  title = "Uso excesivo de las redes sociales Según fallo del servicio fijo" # Titulo con variables.
) + (theme(axis.text.x = element_text(angle = 45, vjust=0.5, size = 8)))
```
...
# Igualdad de varianzas: si p > 0.05 supuesto se cumple
LeveneTest(original_df$obsesion_rrss, original_df$tipo_empresa_internet_fijo)
...
ggbetweenstats(
  ...
  outlier.tagging  = TRUE,
  type = "nonparametric", # cambiar a "parametric" si supuestos se cumplen.
  title = "Obesión de las Redes Sociales Según tipo de empresa de servicio fijo" # Titulo con variables.
) 

# Igualdad de varianzas: si p > 0.05 supuesto se cumple
LeveneTest(original_df$falta_de_control_rrss, original_df$tipo_empresa_internet_fijo)
...
ggbetweenstats(
  ...
  outlier.tagging  = TRUE,
  type = "nonparametric", # cambiar a "parametric" si supuestos se cumplen.
  title = "Falta de control de las Redes Sociales Según tipo de empresa de servicio fijo" # Titulo con variables.
) 

# Igualdad de varianzas: si p > 0.05 supuesto se cumple
LeveneTest(original_df$uso_excesivo_rrss, original_df$tipo_empresa_internet_fijo)
...
ggbetweenstats(
  ...
  outlier.tagging  = TRUE,
  type = "parametric", # cambiar a "parametric" si supuestos se cumplen.
  title = "Uso excesivo de las redes sociales Según tipo de empresa de servicio fijo" # Titulo con variables.
) 
```
...
# Igualdad de varianzas: si p > 0.05 supuesto se cumple
LeveneTest(original_df$obsesion_rrss, original_df$cambio_proveedor_reciente)
...
ggbetweenstats(
  ...
  outlier.tagging  = TRUE,
  type = "nonparametric", # cambiar a "parametric" si supuestos se cumplen.
  title = "Obesión de las Redes Sociales Según el cambio de proveedor" # Titulo con variables.
) 

# Igualdad de varianzas: si p > 0.05 supuesto se cumple
LeveneTest(original_df$falta_de_control_rrss, original_df$cambio_proveedor_reciente)
...
ggbetweenstats(
  ...
  outlier.tagging  = TRUE,
  type = "nonparametric", # cambiar a "parametric" si supuestos se cumplen.
  title = "Falta de control de las Redes Sociales Según el cambio de proveedor" # Titulo con variables.
) 

# Igualdad de varianzas: si p > 0.05 supuesto se cumple
LeveneTest(original_df$uso_excesivo_rrss, original_df$cambio_proveedor_reciente)
...
# Igualdad de varianzas: si p > 0.05 supuesto se cumple
LeveneTest(original_df$obsesion_rrss, original_df$medida_secundaria_fallo_internet_fijo)
...
ggbetweenstats(
  ...
  outlier.tagging  = TRUE,
  type = "nonparametric", # cambiar a "parametric" si supuestos se cumplen.
  title = "Obesión de las Redes Sociales Según opción secundaria para acceder a internet" # Titulo con variables.
) + coord_flip()

# Igualdad de varianzas: si p > 0.05 supuesto se cumple
LeveneTest(original_df$falta_de_control_rrss, original_df$medida_secundaria_fallo_internet_fijo)
...
ggbetweenstats(
  ...
  outlier.tagging  = TRUE,
  type = "nonparametric", # cambiar a "parametric" si supuestos se cumplen.
  title = "Falta de control de las Redes Sociales Según opción secundaria para acceder a internet" # Titulo con variables.
) + coord_flip()

# Igualdad de varianzas: si p > 0.05 supuesto se cumple
LeveneTest(original_df$uso_excesivo_rrss, original_df$medida_secundaria_fallo_internet_fijo)
...
ggbetweenstats(
  ...
  outlier.tagging  = TRUE,
  type = "parametric", # cambiar a "parametric" si supuestos se cumplen.
  title = "Uso excesivo de las redes sociales Según opción secundaria para acceder a internet" # Titulo con variables.
) + coord_flip()
```
...
# Igualdad de varianzas: si p > 0.05 supuesto se cumple
LeveneTest(original_df$obsesion_rrss, original_df$pago_internet_dolares)
...
ggbetweenstats(
  ...
  outlier.tagging  = TRUE,
  type = "nonparametric", # cambiar a "parametric" si supuestos se cumplen.
  title = "Obesión de las Redes Sociales Según pago en dolares" # Titulo con variables.
) 

# Igualdad de varianzas: si p > 0.05 supuesto se cumple
LeveneTest(original_df$falta_de_control_rrss, original_df$pago_internet_dolares)
...
ggbetweenstats(
  ...
  outlier.tagging  = TRUE,
  type = "nonparametric", # cambiar a "parametric" si supuestos se cumplen.
  title = "Falta de control de las Redes Sociales Según pago en dolares" # Titulo con variables.
) 

# Igualdad de varianzas: si p > 0.05 supuesto se cumple
LeveneTest(original_df$uso_excesivo_rrss, original_df$pago_internet_dolares)
...
ggbetweenstats(
  ...
  outlier.tagging  = TRUE,
  type = "parametric", # cambiar a "parametric" si supuestos se cumplen.
  title = "Uso excesivo de las redes sociales Según pago en dolares" # Titulo con variables.
) 
```
...
# Igualdad de varianzas: si p > 0.05 supuesto se cumple
LeveneTest(original_df$obsesion_rrss, original_df$acceso_celular_rrss)

# Normalidad de VD: si p > 0.05 supuesto se cumple
shapiro.test(original_df$obsesion_rrss)

bi_calm2 <-ggbetweenstats(
  data  = original_df,
  x     = acceso_celular_rrss,
  y     = obsesion_rrss,
  outlier.tagging  = TRUE,
  type = "nonparametric", # cambiar a "parametric" si supuestos se cumplen.
  title = "Obesión de las Redes Sociales Según acceso a RRSS vía Celular" # Titulo con variables.
) 

# Igualdad de varianzas: si p > 0.05 supuesto se cumple
LeveneTest(original_df$falta_de_control_rrss, original_df$acceso_celular_rrss)

# Normalidad de VD: si p > 0.05 supuesto se cumple
shapiro.test(original_df$falta_de_control_rrss)

bi_calm3 <-ggbetweenstats(
  data  = original_df,
  x     = acceso_celular_rrss,
  y     = falta_de_control_rrss,
  outlier.tagging  = TRUE,
  type = "nonparametric", # cambiar a "parametric" si supuestos se cumplen.
  title = "Falta de control de las Redes Sociales Según acceso a RRSS vía Celular" # Titulo con variables.
) 

# Igualdad de varianzas: si p > 0.05 supuesto se cumple
LeveneTest(original_df$uso_excesivo_rrss, original_df$acceso_celular_rrss)

# Normalidad de VD: si p > 0.05 supuesto se cumple
shapiro.test(original_df$uso_excesivo_rrss)

bi_calm4 <-ggbetweenstats(
  data  = original_df,
  x     = acceso_celular_rrss,
  y     = uso_excesivo_rrss,
  outlier.tagging  = TRUE,
  type = "parametric", # cambiar a "parametric" si supuestos se cumplen.
  title = "Uso excesivo de las redes sociales Según acceso a RRSS vía Celular" # Titulo con variables.
) 

plot_grid(bi_calm1, bi_calm2, bi_calm3, bi_calm4)
```
...
# Igualdad de varianzas: si p > 0.05 supuesto se cumple
LeveneTest(original_df$obsesion_rrss, original_df$calidad_navegacion_movil)
...
ggbetweenstats(
  ...
  outlier.tagging  = TRUE,
  type = "nonparametric", # cambiar a "parametric" si supuestos se cumplen.
  title = "Obesión de las Redes Sociales Según la calidad de navegación móvil" # Titulo con variables.
) 

# Igualdad de varianzas: si p > 0.05 supuesto se cumple
LeveneTest(original_df$falta_de_control_rrss, original_df$calidad_navegacion_movil)
...
ggbetweenstats(
  ...
  outlier.tagging  = TRUE,
  type = "nonparametric", # cambiar a "parametric" si supuestos se cumplen.
  title = "Falta de control de las Redes Sociales Según la calidad de navegación móvil" # Titulo con variables.
) 

# Igualdad de varianzas: si p > 0.05 supuesto se cumple
LeveneTest(original_df$uso_excesivo_rrss, original_df$calidad_navegacion_movil)
...
ggbetweenstats(
  ...
  outlier.tagging  = TRUE,
  type = "parametric", # cambiar a "parametric" si supuestos se cumplen.
  title = "Uso excesivo de las redes sociales Según la calidad de navegación móvil" # Titulo con variables.
) 
```
...
# Igualdad de varianzas: si p > 0.05 supuesto se cumple
LeveneTest(original_df$obsesion_rrss, original_df$navegacion_movil_positivo)
...
ggbetweenstats(
  ...
  outlier.tagging  = TRUE,
  type = "nonparametric", # cambiar a "parametric" si supuestos se cumplen.
  title = "Obesión de las Redes Sociales Según la percepción positiva de la navegación móvil" # Titulo con variables.
) + coord_flip()

# Igualdad de varianzas: si p > 0.05 supuesto se cumple
LeveneTest(original_df$falta_de_control_rrss, original_df$navegacion_movil_positivo)
...
ggbetweenstats(
  ...
  outlier.tagging  = TRUE,
  type = "nonparametric", # cambiar a "parametric" si supuestos se cumplen.
  title = "Falta de control de las Redes Sociales Según la percepción positiva de la navegación móvil" # Titulo con variables.
) + coord_flip()

# Igualdad de varianzas: si p > 0.05 supuesto se cumple
LeveneTest(original_df$uso_excesivo_rrss, original_df$navegacion_movil_positivo)
...
ggbetweenstats(
  ...
  outlier.tagging  = TRUE,
  type = "parametric", # cambiar a "parametric" si supuestos se cumplen.
  title = "Uso excesivo de las redes sociales Según la percepción positiva de la navegación móvil" # Titulo con variables.
) + coord_flip()
```
...
ggbetweenstats(
  ...
  outlier.tagging  = TRUE,
  type = "nonparametric", # cambiar a "parametric" si supuestos se cumplen.
  title = "Obesión de las Redes Sociales Según Percepción Negativa de la Navegación Móvil" # Titulo con variables.
) + coord_flip()
...
ggbetweenstats(
  ...
  outlier.tagging  = TRUE,
  type = "nonparametric", # cambiar a "parametric" si supuestos se cumplen.
  title = "Falta de control de las Redes Sociales Percepción Negativa de la Navegación Móvil" # Titulo con variables.
) + coord_flip()
...
ggbetweenstats(
  ...
  outlier.tagging  = TRUE,
  type = "parametric", # cambiar a "parametric" si supuestos se cumplen.
  title = "Uso excesivo de las redes sociales Según Percepción Negativa de la Navegación Móvil" # Titulo con variables.
) + coord_flip()
```
...
auto_disp1 <- run_quantitative_bivariate_test(
  xColName = "area_interperso",
  yColName = "adiccion_rrss",
   df = original_df,
  x_label = "Puntaje Autoestima Factor Área Interpersonal",
  y_label = "Puntaje Total Constructo Adicción a las Redes Sociales",
  title = "Relación Constructo ARS vs Área Interpersonal"
)


auto_disp2 <-run_quantitative_bivariate_test(
  xColName = "area_personal_corporal",
 yColName = "adiccion_rrss",
 df = original_df,
  x_label = "Puntaje Autoestima Factor Área Personal Corporal",
  y_label = "Puntaje Total Constructo Adicción a las Redes Sociales",
  title = "Relación Constructo ARS vs Área Personal Corporal"
)

plot_grid(auto_disp1, auto_disp2)
```
...
#Adicción a las Redes Sociales vs Factor insatifacción con la imagen corporal

```{r}

bsq_disp1 <- run_quantitative_bivariate_test(
 xColName = "insatisfaccion_imagen_corp",
 yColName = "adiccion_rrss",
 df = original_df,
 x_label = "Insatisfacción corporal",
 y_label  = "Constructo de Adicción a las Redes Sociales",
 title = "Relación entre la ARS vs Insatisfacción corporal"
)

bsq_disp2 <-run_quantitative_bivariate_test(
 xColName = "preocupacion_peso_conductas_asociadas",
 yColName = "adiccion_rrss",
 df = original_df,
 x_label = "Preocupación por el peso y conductas asociadas",
 y_label  = "Constructo de Adicción a las Redes Sociales",
 title = "Relación entre la ARS vs Preocupación por el peso y conductas asociadas"
)

bsq_disp3 <-run_quantitative_bivariate_test(
 xColName = "factores_antiesteticos_emociones",
 yColName = "adiccion_rrss",
 df = original_df,
 x_label = "Factores Antiestéticos y emocionales",
 y_label  = "Constructo de Adicción a las Redes Sociales",
 title = "Relación entre la ARS vs Factores Antiestéticos y emocionales"
)

plot_grid(bsq_disp1,bsq_disp2, bsq_disp3, ncol = 2, nrwo = 2)
...
model_lr_constructo_total <- lm(adiccion_rrss ~ area_interperso + area_personal_corporal + insatisfaccion_imagen_corp + preocupacion_peso_conductas_asociadas + factores_antiesteticos_emociones, data = original_df)

performance::check_model(model_lr_constructo_total)
summary(model_lr_constructo_total)
...
```{r}
model_lr_obsesion <- lm(obsesion_rrss ~ area_interperso + area_personal_corporal + insatisfaccion_imagen_corp + preocupacion_peso_conductas_asociadas + factores_antiesteticos_emociones, data = original_df)

performance::check_model(model_lr_obsesion)
summary(model_lr_obsesion)
...
```{r}
model_lr_falta_control <- lm(falta_de_control_rrss ~ area_interperso + area_personal_corporal + insatisfaccion_imagen_corp + preocupacion_peso_conductas_asociadas + factores_antiesteticos_emociones, data = original_df)

performance::check_model(model_lr_falta_control)
summary(model_lr_falta_control)
```

Uso Excesivo

```{r}
model_lr_uso_excesivo_rrss <- lm(uso_excesivo_rrss ~ area_interperso + area_personal_corporal + insatisfaccion_imagen_corp + preocupacion_peso_conductas_asociadas + factores_antiesteticos_emociones, data = original_df)

performance::check_model(model_lr_uso_excesivo_rrss)
summary(model_lr_uso_excesivo_rrss)
```

```{r}

#Intento de creación de funcion para crear graficos mas facil
analisis_bivariante <- function
(x,
 y)
{
  LeveneTest(y, x)
...
grafico_bivariante <- function(data,
                               x,
                               y,
                               title = "") {
  ggbetweenstats(
...
analisis_bivariante(
  x = original_df$navegacion_movil_negativo,
  y = original_df$adiccion_rrss)

grafico_bivariante (
  data = original_df,
  x = navegacion_movil_negativo,
  y = adiccion_rrss,
  type = "parametric",
  title = "Hola"
)
