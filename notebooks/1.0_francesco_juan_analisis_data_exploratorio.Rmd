...
#  y = adiccion_rrss,
#  title = "Adicción Redes Sociales Según Percepción Negativa de la Navegación Móvil")
```
...
# Igualdad de varianzas: si p > 0.05 supuesto se cumple
LeveneTest(original_df$obsesion_rrss, original_df$edad)
...
ggbetweenstats(
  ...
  outlier.tagging  = TRUE,
  type = "nonparametric", # cambiar a "parametric" si supuestos se cumplen.
  title = "Obesión de las Redes Sociales Según edad" # Titulo con variables.
)

# Igualdad de varianzas: si p > 0.05 supuesto se cumple
LeveneTest(original_df$falta_de_control_rrss, original_df$edad)
...
ggbetweenstats(
  ...
  outlier.tagging  = TRUE,
  type = "nonparametric", # cambiar a "parametric" si supuestos se cumplen.
  title = "Falta de control de las Redes Sociales Según edad" # Titulo con variables.
)

# Igualdad de varianzas: si p > 0.05 supuesto se cumple
LeveneTest(original_df$uso_excesivo_rrss, original_df$edad)
...
ggbetweenstats(
  ...
  outlier.tagging  = TRUE,
  type = "parametric", # cambiar a "parametric" si supuestos se cumplen.
  title = "Uso excesivo de las redes sociales Según edad" # Titulo con variables.
)
```
...
run_quantitative_bivariate_test <-
  function(x_vector,
    ...
    original_df %>%
      ggplot(aes(x = x_vector, y = y_vector)) +
      geom_point() +
...
auto_disp2 <-run_quantitative_bivariate_test(
  x_vector = original_df$area_personal_corporal,
  y_vector = original_df$adiccion_rrss,
  x_label = "Puntaje Autoestima Factor Area Coporal/Personal",
  y_label = "Puntaje Total Constructo Adicción a las Redes Sociales",
  title = "Relación Constructo ARS vs Área Coporal/Personal"
)
...
bsq_disp1 <- run_quantitative_bivariate_test(
  x_vector = original_df$insatisfaccion_imagen_corp,
  y_vector = original_df$adiccion_rrss,
  x_label = "Insatisfacción en la imagen corporal",
  y_label = "Constructo Adicción a las Redes Sociales",
  title = "ARS vs Insatisfacción de la imagen corporal"
) 

bsq_disp2 <-run_quantitative_bivariate_test(
  x_vector = original_df$preocupacion_peso_conductas_asociadas,
  y_vector = original_df$adiccion_rrss,
  x_label = "Preocupación por el peso y conductas asociadas",
  y_label = "Constructo Adicción a las Redes Sociales",
  title = "ARS vs Preocupación por el peso y coductas asociadas"
) 

bsq_disp3 <-run_quantitative_bivariate_test(
  x_vector = original_df$factores_antiesteticos_emociones,
  y_vector = original_df$adiccion_rrss,
  x_label = "Factores Antiestéticos y emocionales",
  y_label = "Constructo Adicción a las Redes Sociales",
  title = "ARS vs Factores Antiestéticos y emocionales"
) 
...
model_lr_constructo_total <- lm(original_df$adiccion_rrss ~ original_df$area_interperso + original_df$area_personal_corporal + original_df$insatisfaccion_imagen_corp + original_df$preocupacion_peso_conductas_asociadas + original_df$factores_antiesteticos_emociones, data = original_df)

performance::check_model(model_lr_constructo_total)
summary(model_lr_constructo_total)

model_lr_uso_excesivo_rrss <- lm(original_df$uso_excesivo_rrss ~ original_df$area_interperso + original_df$area_personal_corporal)
```
